...
class partner_open_arap_print(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        if context is None:
            context = {}
        super(partner_open_arap_print, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if data.get('fiscalyear_id'):
            ...
            # find periods to select move_lines within FY that are reconciled after FY
        ...
        if data.get('period_id'):
            ...
            period_query_ids = period_obj.search(cr, uid, [('date_stop', '<=', period_date_start), ('company_id', '=', company_id)])
            period_query_ids += [period_id] 
            # find periods to select move_lines that are reconciled after period
            next_period_ids = period_obj.search(cr, uid, [('date_stop', '>', period.date_stop), ('company_id', '=', company_id)])
           
        report_ar = {'type': 'receivable', 'title': title_prefix + _('Open Receivables'), 'title_short': title_short_prefix + ', ' + _('AR')}
        report_ap = {'type': 'payable', 'title': title_prefix + _('Open Payables'), 'title_short': title_short_prefix + ', ' + _('AP')}

        query_start = "SELECT l.move_id as m_id, l.id as l_id, l.date as l_date, " \
            ...
            "p.id as p_id, p.name as p_name, p.ref as p_ref, l.name as l_name, " \
            "l.debit, l.credit, ai.date_due, " \
            "l.reconcile_id, r.name as r_name, " \
            "l.reconcile_partial_id, rp.name as rp_name, " \
            "ai.internal_number as inv_number, b.name as st_number, v.number as voucher_number " \
        ...
        if by_fy:
            move_selection += 'AND account_period.fiscalyear_id in %s ' %str(tuple(fy_query_ids)).replace(',)', ')')
        else:
            move_selection += 'AND account_period.id in %s' %str(tuple(period_query_ids)).replace(',)', ')')
            
        # define subquery to select move_lines within FY/period that are reconciled after FY/period
        if next_period_ids:
        ...
        for report in reports:
            ...
            if all_lines:
                ...
                p_map = map(lambda x: {'p_id': x['p_id'], 'p_name': x['p_name'], 'p_ref': x['p_ref']}, all_lines)
                for p in p_map:
                ...
                sum_debit = 0
                sum_credit = 0
                acc_lines = filter(lambda x: x['a_type'] == report['type'], all_lines)
        ...
        super(partner_open_arap_print, self).set_context(objects, data, ids, report_type)
    
    def formatLang(self, value, digits=None, date=False, date_time=False, grouping=True, monetary=False, dp=False, currency_obj=False):
        if isinstance(value, (float, int)) and not value:
            return ''
        else:
            return super(partner_open_arap_print, self).formatLang(value, digits, date, date_time, grouping, monetary, dp, currency_obj)

report_sxw.report_sxw('report.account.partner.open.arap.period.print', 'account.period', 
    'addons/account_open_receivables_payables_xls/report/account_partner_open_arap.rml', 
